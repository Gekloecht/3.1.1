...
switch (playerSide) do
{
	case west:
	{
		...
		//Lights?
		life_actions = life_actions + [player addAction["Allumer Gyrophare",{[[vehicle player,0.22],"life_fnc_copLights",true,false] spawn life_fnc_MP; vehicle player setVariable["lights",true,true];},"",0,false,false,"",' vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F"] && !isNil {vehicle player getVariable "lights"} && ((driver vehicle player) == player) && !(vehicle player getVariable "lights")']];
		life_actions = life_actions + [player addAction["Eteindre Gyrophare",{vehicle player setVariable["lights",false,true];},"",0,false,false,"", ' vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F"] && !isNil {vehicle player getVariable "lights"} && ((driver vehicle player) == player) && (vehicle player getVariable "lights") ']];
		life_actions = life_actions + [player addAction["Saisir les armes",life_fnc_seizePlayerIllegal,cursorTarget,0,false,false,"",'!isNull cursorTarget && (player distance cursorTarget) < 6 && speed cursorTarget < 2 && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget == civilian) && (cursorTarget getVariable "restrained") && life_coplevel >= 2']];
		...
		//search house      
        life_actions = life_actions + [player addAction["<t color='#FFFF00'>Fouiller Maison</t>",life_fnc_houseOwnerSearch,cursorTarget,5,false,false,"",
        ...
        life_actions = life_actions + [player addAction["<t color='#FF0000'>Raid sur la Maison</t>",life_fnc_raidHouse,cursorTarget,5,false,false,"",
        ...
        life_actions = life_actions + [player addAction["<t color='#FFFF00'>Fouiller inventaire</t>",life_fnc_houseInvSearch,cursorTarget,5,false,false,"",
	...
	case civilian:
	{
		...
		//Grab turtle
		life_actions = life_actions + [player addAction["Pêcher des tortues",life_fnc_catchTurtle,"",1000,false,false,"",'
		!life_action_inUse && (vehicle player isKindOf "Ship") && ((player distance (getMarkerPos "turtle_1") < 200) OR (player distance (getMarkerPos "turtle_2") < 200) OR (player distance (getMarkerPos "turtle_3") < 200)) && !alive cursorTarget && (life_carryWeight + (["turtle"] call life_fnc_itemWeight)) <= life_maxWeight && speed (vehicle player) < 2 && speed (vehicle player) > -1 && !life_net_dropped ']];
		//Gather Cannabis
		...
		//Gathers Diamonds
		life_actions = life_actions + [player addAction["Récolter du diamant",life_fnc_gatherDiamond,"",0,false,false,"",'
		!life_action_in_use && ((player distance (getMarkerPos "diamond_1") < 150) OR (player distance (getMarkerPos "diamond_2") < 150)  OR (player distance (getMarkerPos "diamond_3") < 150) OR (player distance (getMarkerPos "diamond_4") < 150)) && (vehicle player == player) && (life_carryWeight + (["diamond"] call life_fnc_itemWeight)) <= life_maxWeight ']];

		//Gathers Salt
		life_actions = life_actions + [player addAction["Récolter du sel",life_fnc_gatherSalt,"",0,false,false,"",'
		...
		//Gathers Copper
		life_actions = life_actions + [player addAction["Récolter du cuivre",life_fnc_gatherCopper,"",0,false,false,"",'
		!life_action_in_use && ((player distance (getMarkerPos "lead_1") < 150) OR (player distance (getMarkerPos "lead_2") < 150) OR (player distance (getMarkerPos "lead_3") < 150) OR (player distance (getMarkerPos "lead_4") < 150)) && (vehicle player == player) && (life_carryWeight + (["copperore"] call life_fnc_itemWeight)) <= life_maxWeight ']];

		//Gathers Iron
		life_actions = life_actions + [player addAction["Récolter du fer",life_fnc_gatherIron,"",0,false,false,"",'
		!life_action_in_use && ((player distance (getMarkerPos "iron_1") < 150) OR (player distance (getMarkerPos "iron_2") < 150) OR (player distance (getMarkerPos "iron_3") < 150) OR (player distance (getMarkerPos "iron_4") < 150)) && (vehicle player == player) && (life_carryWeight + (["ironore"] call life_fnc_itemWeight)) <= life_maxWeight ']];

		//Gathers Sand
		life_actions = life_actions + [player addAction["Récolter du sable",life_fnc_gatherSand,"",0,false,false,"",'
		!life_action_in_use && ((player distance (getMarkerPos "sand_1") < 150) OR (player distance (getMarkerPos "sand_2") < 150) OR (player distance (getMarkerPos "sand_3") < 150) OR (player distance (getMarkerPos "sand_4") < 150)) && (vehicle player == player) && (life_carryWeight + (["sand"] call life_fnc_itemWeight)) <= life_maxWeight ']];
